...
class UserDB:
    ...
    def get_all_users_detailed(self) -> List[Dict]:
        """Получает детальную информацию о всех пользователях для экспорта в Excel"""
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT u.user_id, u.username, u.first_name, u.created_at,
                   s.trial_end, s.paid_until, s.is_forever,
                   (SELECT COUNT(*) FROM referrals WHERE referrer_id = u.user_id) as total_refs,
                   (SELECT COALESCE(SUM(commission_earned), 0) FROM referrals WHERE referrer_id = u.user_id) as total_commission
            FROM users u
            LEFT JOIN subscriptions s ON u.user_id = s.user_id
            ORDER BY u.created_at DESC
        ''')
        ...
        for r in rows:
            ...
            if r[6]:  # is_forever
                ...
                paid = date.fromisoformat(r[5])
                if paid >= today:
                    days_left = (paid - today).days
            elif r[4]:  # trial_end
                trial = date.fromisoformat(r[4])
                if trial >= today:
